...
if (isset($_POST['add_books'])) {
  ...
  if (empty($bname)) {
   ...
   $message[] = 'Please Choose Document File';
  }else {

    $add_book = mysqli_query($conn, "INSERT INTO book_info(`name`, `title`, `price`, `category`, `description`, `image`, `pdf`) VALUES('$bname','$btitle','$price','$category','$desc','$img','$pdf')") or die('Query failed');

    if ($add_book) {

      move_uploaded_file($img_temp_name, $img_file);
      move_uploaded_file($pdf_temp_name, $pdf_file);
...
if(isset($_POST['update_product'])){
  ...
  $update_folder = './added_books/'.$update_pdf;
  $update_old_pdf = $_POST['update_old_pdf'];

  if(!empty($update_pdf)){
     if($update_pdf_size > 2000000){
        $message[] = 'Document file size is too large';
     }else{
        mysqli_query($conn, "UPDATE `book_info` SET pdf = '$update_pdf' WHERE bid = '$update_p_id'") or die('query failed');
        move_uploaded_file($update_pdf_tmp_name, $update_folder);
        unlink('uploaded_img/'.$update_old_pdf);
     }
  }
...
<a class="update_btn" style="position: fixed ; z-index:100;" href="total_books.php">See All Books</a>
  <div class="container_box">
    <form action="" method="POST" enctype="multipart/form-data">
      ...
      <p style="margin-top: 20px;color:black;font-size:small;"><b style="font-size: medium;">Upload Book Picture:</b> .png, .jpg</p>
      <input type="file" name="image" accept="image/jpg, image/jpeg, image/png" class="text_field">
      <p style="margin-top: 20px;color:black;font-size:small;"><b style="font-size: medium;">Upload Book File:</b> .pdf, .docx, .txt</p>
      <input type="file" name="pdf" class="text_field">
...
<form action="" method="post" enctype="multipart/form-data">
   ...
   <input type="file" class="box" name="update_pdf" accept="">
      ...
      <div class="box">
         ...
         <div style="margin-bottom: 25px;">
            <button style="
               ...
               cursor: pointer" 
               onclick="location.href='added_books/<?php echo $fetch_book['pdf']; ?>'">View This Book</button>
         </div>
